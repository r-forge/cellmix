...
<<csplot, fig.height=8, fig.width=7>>=
csplot(rescs)
@
...
We illustrate its use on a small simulated dataset, as it is very computationally intensive: for large datasets, one would need several hundreds/thousands of samples, as well as a burnin period, to ensure the convergence of the Markov chain.
...
its underlying statistical model.

<<DSection, cache=TRUE, fig.show='hold', out.width="0.5\\textwidth">>=
