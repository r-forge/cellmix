...
#' 
#' @details
#' This function calls \code{\link[GEOquery]{getGEO}} in a special environment, 
#' where a custom modified version of \code{\link{getAndParseGSEMatrices}} is defined 
#' to cope with the annoying proxy case.
#' 
...
getGSE <- function(GEO = NULL, filename = NULL, destdir = GEDtmp(), ..., simplify=TRUE)
{
	GEO <- toupper(GEO)
	# require GEOquery
	uq_requirePackage('GEOquery', load=TRUE, msg=str_c("for working with GEO datasets."))
